...
module PerfMetrics
    ...
    require_relative File.join(SourcePath, 'PerfMetricsEngine.rb')
    require_relative File.join(SourcePath, 'PerfMetricIDataCollector.rb')
    ...
    class MetricsEngineConfiguration_test < Test::Unit::TestCase
        ...
        def test_configure_default_values
            assert_equal DefaultPoll, @object_under_test.poll
            assert_nil @object_under_test.computer
        end

        def test_config_poll_nil
            assert_raise ArgumentError do @object_under_test.poll = nil; end
            assert_equal DefaultPoll, @object_under_test.poll
        end

        def test_config_poll_not_number
            assert_raise ArgumentError do @object_under_test.poll = "1"; end
            assert_equal DefaultPoll, @object_under_test.poll
        end

        def test_config_poll_negative
            assert_raise ArgumentError do @object_under_test.poll = -1; end
            assert_equal DefaultPoll, @object_under_test.poll
        end

        def test_config_poll_zero
            assert_raise ArgumentError do @object_under_test.poll = 0; end
            assert_equal DefaultPoll, @object_under_test.poll
        end

        def test_config_poll_half
            assert_raise ArgumentError do @object_under_test.poll = 0.5; end
            assert_equal DefaultPoll, @object_under_test.poll
        end

        def test_config_poll_less_than_1
            assert_raise ArgumentError do @object_under_test.poll = 0.9999999999; end
            assert_equal DefaultPoll, @object_under_test.poll
        end

        def test_config_poll_complex
            assert_raise ArgumentError do @object_under_test.poll = 1+0i; end
            assert_equal DefaultPoll, @object_under_test.poll
        end

        def test_config_poll_1
            @object_under_test.poll = 1
            assert_equal 1, @object_under_test.poll
        end
